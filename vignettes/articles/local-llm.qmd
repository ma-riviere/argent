...
This article covers using local LLM servers with argent. 
The `LocalLLM` provider was tested with llama.cpp (llama-server), but it should work with any OpenAI-compatible local server.
...
*Wrong answer, but it used the tools and returned a structured output. Not too bad for a ~1 year old small local model.*
...
At the minimum, you can send any type of text content (and files/objects automatically converted to text).
