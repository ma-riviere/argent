...
```{r}
cat(anthropic$get_supplementary(format_to = "yaml"))
```
...
Let's send two PDF URLs and use the `as_text_content()` helper to have `pdftools::pdf_convert()` parse the PDFs and pass the text content to the model instead of sending the URLs as-is to the server.
