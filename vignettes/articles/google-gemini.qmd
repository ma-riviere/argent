...
It supports fetching text (HTML, JSON, XML), images (PNG, JPEG, WebP), and PDFs.
...
With Google Gemini, we can create a file search store and use the file_search tool to search through uploaded files, effectively turning it into a server-side RAG application. See the [File Search & RAG](#server-side-rag) section below for a complete example.
...
# File Search & RAG {#server-side-rag}
