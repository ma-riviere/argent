...
```{=html}
<details>
<summary>TODO-list Tools Implementation</summary>
```

`r knitr::spin_child(here::here("inst", "examples", "todo_tools.R"))`
...
- [x] Research all LLM-adjacent packages in R
- [-] Identify the package name, repository URL, description, and dependencies
...
::: {.callout-note appearance="simple"}
See the [Using MCP Servers with `argent`](/articles/advanced-mcp) article for more details on how to use MCP servers with `argent`.
:::
...
You can also define tools manually using `tool()` with nested list structures:
