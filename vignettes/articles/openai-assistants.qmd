...
This does not create an assistant, it only creates an instance of the `OpenAI_Assistant` class. 
For OpenAI Assistants, you need to specifically create an assistant with `create_assistant()` before being able to call the `chat()` method.
:::
