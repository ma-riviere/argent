---
title: "Getting Started with `argent`"
format: html
execute:
  echo: true
  eval: false
...
Here is a quick showcase of the various features of `argent` using the Google (Gemini) provider.
...
- [Using Other Compatible APIs](articles/other-providers.html) - Use argent with providers offering compatible APIs (e.g., Minimax, Qwen, ...)

### Advanced Topics

- [RAG](articles/basic-rag.html) - How to use `argent` & `ragnar` for RAG
- [MCP: Connecting to MCP Servers](articles/mcp-client.html) - How to connect to MCP servers with `argent`
- [MCP: Creating MCP Servers](articles/mcp-server.html) - How to create your own MCP servers with `argent`
- [Argentic Workflow](articles/argentic-workflow.html) - How to use `argent` for an advanced Agentic Workflows
...
::: {.callout-note}
The `fn` parameter stores the function implementation with the tool definition. This is important for:

- **Closures**: Functions that capture local variables (e.g., within another function)
- **Consistency**: Ensures the exact function is called, not a different one with the same name
- **Portability**: The tool carries its implementation, not just a reference to a global name

If `fn` is not provided, argent will look up the function by name in the global environment when the LLM calls it.
:::
